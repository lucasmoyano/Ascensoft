app.controller('technicianSearchController', function($scope, $rootScope, $http, $location) {
	...
	$scope.loadPage = function() {
		...
		$rootScope.helper.ajax('/technician/search.json?' + search + pagination
			, ''
			, function(result) {
	...
	$scope.insert = function(element) {
		...
		$rootScope.helper.ajax('/technician/update.json'
	...
	$scope.remove = function(element) {
		...
		$rootScope.modal.remove(element.first_name + ' ' + element.last_name, function() {
			element.$$isUpdating = true;
			$rootScope.helper.ajax('/technician/remove.json'
				, { model : element }
				, function(result) {
					var index = $scope.elements.indexOf(element);

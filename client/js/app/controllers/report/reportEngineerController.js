app.controller('reportEngineerController', function($scope, $rootScope, $http, $location) {
	...
	$scope.init = function() {
		...
		$scope.loadPage();
		$scope.canvas = $('#engineer #canvas').clone();
	};

	$scope.loadPage = function() {

		$scope.isLoading = true;
		$rootScope.helper.ajax('/report/engineer.json?init=' + $scope.initDate + '&end=' + $scope.endDate
			, ''
			, function(result) {
  ...
  $scope.getColor = function(idx) {
    ...
    $parent = $('#engineer #canvas').parent();
    $('#engineer #canvas').remove();
    $parent.append($scope.canvas.clone());

    var ctx = $('#engineer #canvas').get(0).getContext("2d");

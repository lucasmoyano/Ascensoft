app.controller('accountingEntryClientController', function($scope, $rootScope, $http, $location) {
	...
	$scope.initOnLoaded = function() {
	  ...
	  var eventCompleteLoadAccountingEntry = $rootScope.$on('completeLoadTicket', function(event, client) {
			$scope.client = client;
			$scope.init();
    });
    $scope.$on('$destroy', function() {
    	eventCompleteLoadAccountingEntry();
		});
	}
	...
	$scope.loadPage = function() {
		...
		$rootScope.helper.ajax('/accountingEntry/search.json?' + search + pagination + idClientSearch + paymentDateSearch + sort
			...
			, function(result) {
				...
				$rootScope.$broadcast('completeLoadAccountingEntry', $scope.client);
			}
		);
	};
